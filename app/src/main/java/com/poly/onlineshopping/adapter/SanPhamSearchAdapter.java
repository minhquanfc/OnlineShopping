...
public class SanPhamSearchAdapter extends ArrayAdapter<SanPham> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        Glide.with(imgSearch.getContext()).load(product.getAnhsanpham()).into(imgSearch);
        tvSearchName.setText(product.getTensanpham());
        tvSearchPrice.setText(product.getGiasanpham()+"đ");
        return convertView;
    }
    @NonNull
    @Override
    public Filter getFilter() {
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (constraint == null || constraint.length() == 0) {
                    ...
                    for (SanPham p : sanPhamList) {
                        if (p.getTensanpham().toLowerCase().contains(filter)) {
                            listSuggest.add(p);
                        }
                    }
                }
            ...
            public CharSequence convertResultToString(Object resultValue) {
                return ((SanPham) resultValue).getTensanpham();
            }
        };
    }
}
